#!/usr/bin/env python3
"""
Simple CLI tool to ask questions to the original SmolLM3-3B model (not fine-tuned).

Usage:
    python ask_original_model.py "How do I check my car's oil level?"
    python ask_original_model.py "How do I fix a leaky faucet?"
"""
import sys
import os
import torch
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python ask_original_model.py \"Your question here\"")
        print("\nExamples:")
        print('  python ask_original_model.py "How often should I check my oil level?"')
        print('  python ask_original_model.py "How do I fix a leaky faucet?"')
        print('  python ask_original_model.py "What should I do if my computer won\'t start?"')
        sys.exit(1)
